...
abstract class PaymentTypeControllerBase with Store {
  ...
  @action
  void chageFilter(bool? enabled) => _filterEnabled = enabled;
  ...
  Future<void> loadPayments() async {
    try {
      ...
      log('Erro ao carregar as  informaçõesm de pagamento', error: e, stackTrace: s);
      _errorMessage = 'Erro ao carregar as  informaçõesm de pagamento';
    }
  }
  ...
  Future<void> savePayment({int? id, required String name, required String acronym, required bool enabled}) async {
    _status = PaymentTypeStateStatus.loading;
    final paymenTypeModel = PaymentTypeModel(id: id, name: name, acronym: acronym, enabled: enabled);
    await _paymentTypeRepository.save(paymenTypeModel);
